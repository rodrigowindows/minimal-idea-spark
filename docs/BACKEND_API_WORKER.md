...
O Night Worker possui **2 APIs com contratos HTTP diferentes**:

#### 1. Supabase Edge Function (Fonte de Verdade)
- **Localização**: `minimal-idea-spark/supabase/functions/nightworker-prompts/index.ts`
- **Endpoints**:
  - GET `/health`, `/prompts`, `/prompts/:id` (com events)
  - POST `/prompts`
  - PATCH `/prompts/:id` (service-role only)
- **Campos**: `result_path`, `result_content`, `events[]`, `created_at`, `updated_at`, etc
- **Uso**: Produção, worker.py com `supabase_mode=true`

#### 2. api_server.py (File-based - Alternativo)
- **Localização**: `claude-auto/api_server.py`
- **Endpoints diferentes**:
  - GET `/prompts/:id/status` ⚠️ (não `/prompts/:id`)
  - Sem PATCH, sem /logs
...
- **Modo Supabase** (`supabase_mode=true`): Consome Edge (GET pending + PATCH done/failed)
- **Modo Local** (`supabase_mode=false`): File-based (input/*.txt)

### Frontend Compatibility

O frontend (`src/hooks/useNightWorkerApi.ts`) implementa **fallback automático**:

1. Tenta Edge: `GET /prompts/:id`
...
**Problema**: Primeira linha sempre virava `files_line`, prompts de 1 linha ficavam com `prompt_text=""`.

**Solução** (linhas 297-313 e 576-592):
```python
# Detecta se primeira linha parece path
looks_like_path = bool(first_line and (
    '/' in first_line or '\\' in first_line or ':' in first_line or
    ',' in first_line or
    any(first_line.lower().endswith(ext) for ext in ['.py', '.js', ...])
))
...
#### Worker Developers
- [ ] Usar `supabase_mode=true` para Edge Function
- [ ] Configurar `SUPABASE_SERVICE_ROLE_KEY` (não anon key)
- [ ] Tratar 409 como sucesso (idempotência)
- [ ] Parsing detecta path automaticamente (não quebra prompts de 1 linha)

#### Backend Developers
- [ ] Edge: PATCH só aceita service-role (403 para anon)
- [ ] Edge: Retornar 409 se já processado (idempotente)
- [ ] api_server.py: Endpoint é `/prompts/:id/status` (não `/prompts/:id`)

### Notas Importantes

- **Edge não tem /logs**: 404 tratado silenciosamente pelo frontend
- **PATCH é idempotente**: 409 significa "já processado" (OK para worker)
- **Service-role obrigatório**: PATCH com anon key retorna 403
