...
### Arquitetura de Referência (Fonte de Verdade)

O Night Worker opera com uma hierarquia clara de backends:
...
1. **Supabase Edge Function (ESTA API)** - **FONTE DE VERDADE**
   ...
   - **Autenticação**: Opcional para leitura; `service-role` obrigatório para PATCH (worker).
   - **Logs**: O endpoint `/logs` não existe nesta API (retorna 404).

2. **api_server.py (File-based)** - API Alternativa
   - Localização: `claude-auto/api_server.py`
   - **Divergências de Contrato**:
     - Detalhe: Usa `/prompts/:id/status` (não `/prompts/:id`).
     - Campos: Usa `result` (em vez de `result_content`) e `path` (em vez de `result_path`).
     - Persistência: Salva arquivos diretamente em `input/`.
   - **Uso**: Testes locais e legacy.

3. **worker.py** - Processador real
   - Localização: `claude-auto/night-worker/worker.py`
   - Quando `supabase_mode=true`, atua como consumidor da **Edge Function** (GET pending + PATCH done/failed).

### Frontend Fallback Strategy

O frontend implementa suporte transparente para ambos os backends:
- **Listagem**: Compatível com ambos.
- **Detalhes**: Se `GET /prompts/:id` retornar 404, o frontend tenta automaticamente `GET /prompts/:id/status` e mapeia os campos legados para o formato padrão.
- **Logs**: Falha em `/logs` (404) é tratada como esperada, exibindo um aviso amigável ao usuário.
